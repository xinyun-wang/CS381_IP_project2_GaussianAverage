...
public:
  ...
  void mirrorFraming() {
    for (int i = 0; i < numC+2; i++) {
      ...
      mirrorFramedAry[numR+2][i] = mirrorFramedAry[numR + 1][i];
      mirrorFramedAry[numR + 3][i] = mirrorFramedAry[numR +1][i];
    ...
    for (int i = 0; i < numR+4; i++) {
      ...
      mirrorFramedAry[i][numC+2] = mirrorFramedAry[i][numC + 1];
      mirrorFramedAry[i][numC + 3] = mirrorFramedAry[i][numC + 1];
  ...
  void loadNeighborAry(int i, int j) {
    ...
    for (int x = -2; x < 3; x++) {
        for (int y = -2; y < 3; y++) {
            neighborAry[z] = mirrorFramedAry[x + i][y + j];
            z++;
        }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  e.mirrorFramedAry = new int*[(e.numR+4)];
  e.avgAry = new int*[(e.numR+4)]();
  e.GaussAry = new int*[(e.numR+4)]();
  for (int i = 0; i < e.numR + 4; i++) {
      e.mirrorFramedAry[i] = new int[e.numC + 4];
      e.avgAry[i] = new int[e.numC + 4];
      e.GaussAry[i] = new int[e.numC + 4];
  }
